...
final class JaText
{
    ...
    /**
     * 受講生メモの本文(コーチ / 管理者が残す日常観察)。
     *
     * MEETING_MEMOS と分けているのは原典が両者を明確に区別しているため——
     * 面談メモは「面談に紐づく議事メモ」、受講生メモは「面談以外の日常観察」。
     * 同じ文面が両方に出ると、証跡の画面で違いが伝わらなくなる。
     */
    private const COACH_NOTES = [
        'チャットの返信が数日空くようになっています。次回面談で学習時間の確保状況を確認します。',
        'Q&A で正規化の論点に繰り返しつまずいています。該当章の補足資料を案内しました。',
        '模試の点数は伸びていますが、時間配分に課題が残ります。演習の進め方を見直してもらいます。',
        '受験日が近づいて焦りが見えます。残りの範囲を絞り込み、優先順位を一緒に決めました。',
        '学習の習慣は安定しています。応用問題に進む準備ができていると判断しました。',
        '前回の面談以降、演習の提出ペースが上がりました。本人も手応えを感じている様子です。',
    ];
    ...
    /** 受講生メモの本文(コーチ / 管理者の日常観察) */
    public static function coachNote(): string
    {
        return fake()->randomElement(self::COACH_NOTES);
    }
